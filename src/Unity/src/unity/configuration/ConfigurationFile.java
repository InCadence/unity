...
public class ConfigurationFile implements IConfigurationFile {
	...
	protected Boolean openFromRestoreFile(String fileName) {
		...
		try {
			...
			//Persist Filename (use the actual Filename, so that when we save we save back to the Filename, not over the Restore Filename.
	...
	@Override
	public String getSetting(String settingPath, String defaultValue,
	...
	protected ConfigurationNode getSettingNode(String settingPath, Boolean createIfMissing){
		...
		if (sectionNode == null){
			...
			if(settingNodeList.isEmpty()) {			
				...
				//section node has children, loop through all of child nodes to find setting node that has the input setting path
